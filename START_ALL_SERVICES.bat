...
echo [1/4] Starting Workflow Frontend (Port 5173)...
start "Workflow Frontend" cmd /k "cd workflow-blackhole\client && npm run dev"
timeout /t 3 /nobreak >nul

echo [2/4] Starting Workflow Backend (Port 5001)...
start "Workflow Backend" cmd /k "cd workflow-blackhole\server && npm start"
timeout /t 3 /nobreak >nul

echo [3/4] Starting AI CRM Backend (Port 8000)...
start "AI CRM Backend" cmd /k "cd ai-crm\backend-nodejs && npm start"
timeout /t 3 /nobreak >nul

echo [4/4] Starting AI CRM Frontend (Port 3000)...
...
echo 5. Login to Workflow dashboard (embedded)
echo.
